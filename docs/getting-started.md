...
- `"MySQL"` is the type name of an implementation. The jar registers two, `"MySQL"` and `"JDBC"`, and
  the name is matched exactly, so a database *product* name is not one of them; see
