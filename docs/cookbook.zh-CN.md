...
下面每段代码在 [`docs/examples/cookbook.sk`](examples/cookbook.sk) 里都有一份副本，CI 会在真实服务端上解析它，
所以一段代码不可能教出插件没有的语法。那份副本有两处故意的不同：每段都包在 `command /example-…` 里（没人会执行它，
命令名必须在所有示例文件之间唯一，因为它们都在同一个测试服上加载），消息用的是英文页里的那些。
