...
internal object WriteValues {
    ...
    fun rows(event: Event, trigger: Trigger, variable: Variable<*>, target: DatabaseWork.Target): List<Map<String, Any?>>? =
        try {
    ...
    fun singleRow(
        ...
        val rows = rows(event, trigger, variable, target) ?: return null
