...
import io.github.heyhey123.xiaojieorm.type.nbt.standin.NBTContainer
...
class NbtSupportStandInTest {
    ...
    private companion object {
        const val STAND_IN_ROOT = "io.github.heyhey123.xiaojieorm.type.nbt.standin"
    }
    ...
    fun `a compound is recognised and normalised into a detached one`() {
        val value = NBTContainer("{xiaojie:1}")
    ...
    fun `bytes written from a compound are read back as the same compound`() {
        ...
        assertEquals("{xiaojie:1}", NbtSupport.snbt(read))
    }

    @Test
    fun `snbt parses back into a compound that renders the same text`() {
        assertEquals("{xiaojie:1}", NbtSupport.snbt(NbtSupport.parseSnbt("{xiaojie:1}")))
    }
}
