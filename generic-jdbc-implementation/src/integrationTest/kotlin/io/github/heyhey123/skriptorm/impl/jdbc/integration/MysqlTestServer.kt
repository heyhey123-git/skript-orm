 ...
 *
 * Resolution order:
 * 1. An externally managed server, configured through the `skriptorm.test.mysql.*` system properties
 *    or their `SKRIPTORM_TEST_MYSQL_*` environment equivalents.
...
object MysqlTestServer {
    ...
    const val URL_PROPERTY = "skriptorm.test.mysql.url"
    const val USERNAME_PROPERTY = "skriptorm.test.mysql.username"
    const val PASSWORD_PROPERTY = "skriptorm.test.mysql.password"
    const val DRIVER_PROPERTY = "skriptorm.test.mysql.driver"
    const val IMAGE_PROPERTY = "skriptorm.test.mysql.image"

    const val URL_ENV = "SKRIPTORM_TEST_MYSQL_URL"
    const val USERNAME_ENV = "SKRIPTORM_TEST_MYSQL_USERNAME"
    const val PASSWORD_ENV = "SKRIPTORM_TEST_MYSQL_PASSWORD"
    const val DRIVER_ENV = "SKRIPTORM_TEST_MYSQL_DRIVER"
    const val IMAGE_ENV = "SKRIPTORM_TEST_MYSQL_IMAGE"
    ...
    private fun startContainer(): Endpoint {
        ...
        Runtime.getRuntime().addShutdownHook(
            Thread({ runCatching { container.stop() } }, "skriptorm-mysql-container-shutdown")
        )
        return Endpoint(
