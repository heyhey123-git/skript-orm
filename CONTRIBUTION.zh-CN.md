...
是 `MySQL`；`PostgreSQL` 与 `MongoDB` 会让同一批脚本分别对着这两个实现跑，这正是它们各自 CI job 做的事。这个名字
同时也决定凭据从哪组键读取，所以一套 `-P` 就同时配置了集成测试与这一层；不属于任何一边的名字会让构建失败，而不是
悄悄在没有数据库的情况下运行。

这带来三个写法上的后果。有连接之后，每个元素报告的内容取决于它走到了哪一步，所以该模式下它们的期望消息为空、
只检查「是否出现」，真正的断言由 roundtrip 承担。setup 与 roundtrip 脚本同时保留「启动守卫」和「完成标记」——
两者不是一回事：它们的步骤会在数据库上等待，没有守卫的话，第二次触发会在第一次还没做完时开始，把同一张表注册
两次。最后，没有事务的实现用的是**同一个**事务脚本：`database transaction` 段会拒绝开启事务、跳过它的主体并继续
往下走，所以脚本照样跑到结尾，只是报告那句拒绝而不是提交——这正是 `MongoDB` 的期望值，也是两种模式对同一个脚本
期望不同消息的唯一一处。

### 持续集成

`.github/workflows/ci.yml` 运行七个 job：
...
- `mongo-server`：对着 MongoDB 跑 Skript 服务端测试。这里的 MongoDB 是这个 job 的 **service 容器**，而不是
  Testcontainers 管理的容器：被测进程是构建启动的 Paper 服务端，它需要一个在运行开始前就已经在应答的地址，
  所以这个 job 会用 `mongosh` 等它就绪，就像 PostgreSQL 那个 job 用 `pg_isready` 等一样。它也是唯一一个
  事务相关期望值是「不支持事务」那句拒绝的 job。和另外几个容器 job 一样，它只在默认分支和手动触发时运行。
